...
// Precautions
//
// Tips: Fix compiler errors based on compiler output, understand precautions
...
struct Object { // Do not modify the code of this class
  ...
  std::string name;
  Object() { construction_counter++; }

  Object(std::string name_) : name{name_} { construction_counter++; }
};
...
public:
  ...
  Account(std::string id_, std::string name_, int coin_)
      : id{id_}, name{name_}, coin{std::to_string(coin_) + "元"}, obj {
    name_
  } {
    // id = id_;
    // name = name_;
...
std::string get_object_name() const { return obj.name; }

std::string to_string() const {
  return "Account { id: " + id + ", name: " + name + ", coin: " + coin +
         ", Object { name: " + obj.name + ", construction_counter: " +
         std::to_string(Object::construction_counter) + " } }";
}
}
;
